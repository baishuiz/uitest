...
var Pattern = function (pattern, served, included, watched) {
	...
	this.served = helper.isDefined(served)
		? served
		: true;
	this.included = helper.isDefined(included)
		? included
		: true;
	this.watched = helper.isDefined(watched)
		? watched
		: true;
};
...
var parseConfig = function (configFilePath, cliOptions) {
	...
	var config = {
		...
		server          : constant.DEFAULT_SERVER,
		serverPort      : constant.DEFAULT_SERVERPORT,
		remoteServerPort: constant.DEFAULT_REMOTESERVERPORT,
		colors          : true,
		autoWatch       : false,
		reporters       : ['progress'],
		singleRun       : true,
	...
	Object.getOwnPropertyNames(cliOptions).forEach(function (key) {
		if (cliOptions.hasOwnProperty(key)) {
			config[key] = cliOptions[key];
		}
	});
