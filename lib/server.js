...
//建立浏览器链接服务
    ...
    log.info('connect to %s:%s', config.server, config.serverPort);
    ...
    socket.on('console:task_start', function (data) {
        ...
        if (data.func && data.pageUrl) {

            runner.run({func: data.func, pageUrl: data.pageUrl, singleRun: false, browser: data.browser}, complete)
        }
        else if (data.url) {
