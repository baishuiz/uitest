...
var Driver = function (id, emitter, captureTimeout) {
	...
	this.executeAsync = function (scripts, callback) {
		...
		// scripts = 'document.body.style.backgroundColor="red";setTimeout(function(){arguments[arguments.length-1]("123")},1000) ';

		var body = JSON.stringify({script: scripts, args: []});
		...
		var req = http.request(options, function (response) {
			...
			log.debug('executeAsync code:%s', response.statusCode);
	...
	this.navigator = function (url, func) {
		...
		var runFunc = function () {
			self.timeout(function () {
				self.url(self.capturingUrl, function (cc) {
					log.debug('timeout OK');
					self.executeAsync(func, function (result) {

						log.debug(result);

						uitest.setData(result.shareData);
