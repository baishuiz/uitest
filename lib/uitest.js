...
Queue.prototype.next_ = function () {
	...
	log.debug('Running %s block', self.index + 1);
...
var addQueue = function (cfg) {
	log.debug('add queue:%s', JSON.stringify(cfg));
	var env = getEnv(),
		runner = env.runner,
		navBlock;
	if (cfg.url) {
...
function _login(username, password) {
	var src = 'https://login.' + domain + '/member/login.jhtml?from=buy' +
...
function _logout() {
	var src = 'https://login.' + domain + '/member/login.jhtml?style=minisimple&from=buy' +
